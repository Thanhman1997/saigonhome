...
export function ReviewsSection({ reviews }: { reviews: Array<{ id: number; customerName: string; rating: number; comment: string; commentEn?: string | null; commentKo?: string | null; commentVi?: string | null }> }) {
  ...
  const localizedComment = (review: (typeof reviews)[number]) => {
    const localized = review[locale === "en" ? "commentEn" : locale === "ko" ? "commentKo" : "commentVi"]
    if (localized) return localized
    if (locale === "vi") return "Bản đánh giá này chưa có bản dịch tiếng Việt."
    if (locale === "ko") return "이 리뷰는 아직 한국어로 번역되지 않았습니다."
    return "This review has not been translated into English yet."
  }
