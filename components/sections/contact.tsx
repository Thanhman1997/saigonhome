...
export function ContactSection({ contactInfo }: { contactInfo: ContactInfoRow }) {
  ...
  return <section id="contact" className="bg-accent py-20 font-sans text-accent-foreground lg:py-24"><div className="mx-auto max-w-[88rem] px-5 lg:px-16"><div className="mx-auto max-w-3xl text-center"><p className="font-sans text-sm font-semibold uppercase tracking-[0.22em] text-accent-foreground sm:text-base">{t.kicker}</p><h2 className="mt-5 text-balance font-sans text-lg font-semibold leading-relaxed tracking-tight sm:text-xl lg:text-2xl">{t.title}</h2>{t.subtitle && <p className="mx-auto mt-5 max-w-2xl text-sm leading-relaxed text-accent-foreground/80 sm:text-base">{t.subtitle}</p>}</div>{channels.length > 0 && <div className="mx-auto mt-8 flex max-w-4xl flex-wrap justify-center gap-x-6 gap-y-4">{channels.map((c) => <a key={c.label} href={c.href} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 rounded-full border border-accent-foreground/20 px-4 py-2 text-sm font-medium text-accent-foreground/90 transition-all hover:-translate-y-0.5 hover:border-accent-foreground/45 hover:text-accent-foreground sm:text-base">{c.icon}{c.label}</a>)}</div>}</div></section>
}
