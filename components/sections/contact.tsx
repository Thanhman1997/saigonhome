...
export function ContactSection({ contactInfo }: { contactInfo: ContactInfoRow }) {
  ...
  return <section id="contact" className="bg-muted py-16 font-sans text-foreground lg:py-20"><div className="mx-auto max-w-5xl px-5 lg:px-8"><div className="mx-auto max-w-3xl text-center"><p className="font-sans text-2xl font-bold uppercase tracking-[0.14em] text-foreground sm:text-3xl">{t.kicker}</p><h2 className="mt-5 text-balance font-sans text-lg font-semibold leading-relaxed tracking-tight sm:text-xl lg:text-2xl">{t.title}</h2>{t.subtitle && <p className="mx-auto mt-5 max-w-2xl text-[2px] leading-relaxed text-foreground/80">{t.subtitle}</p>}</div>{channels.length > 0 && <div className="mx-auto mt-8 flex max-w-4xl flex-wrap justify-center gap-x-6 gap-y-4">{channels.map((c) => <a key={c.label} href={c.href} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 text-sm font-medium text-foreground/95 transition-colors hover:text-foreground sm:text-base">{c.icon}{c.label}</a>)}</div>}</div></section>
}
