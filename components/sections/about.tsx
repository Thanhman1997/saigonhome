        ...
        <div className="mt-16 grid items-stretch gap-6 lg:grid-cols-[1.2fr_.8fr]">
          ...
          <div className="grid gap-3">
            {values.length > 0 ? values.map((value, index) => {
              ...
              return <article key={value.id} className="group flex min-h-32 w-full items-center gap-3 rounded-2xl bg-accent/10 px-3 py-3 transition-all duration-500 hover:-translate-y-1 hover:bg-accent/15"><div className="relative size-32 shrink-0 overflow-hidden rounded-full ring-2 ring-card"><Image src={valueImages[index % valueImages.length]} alt="" fill className="object-cover transition-transform duration-500 group-hover:scale-110" /></div><div className="flex min-w-0 items-center gap-2"><Icon className="size-5 shrink-0 text-accent" aria-hidden="true" /><h3 className="whitespace-nowrap font-serif text-3xl font-semibold leading-tight text-accent">{text}</h3></div></article>
            }) : t.about.values.map((value, index) => {
              const Icon = fallbackIcons[index]
              return <article key={value} className="group flex min-h-32 w-full items-center gap-3 rounded-2xl bg-accent/10 px-3 py-3 transition-all duration-500 hover:-translate-y-1 hover:bg-accent/15"><div className="relative size-32 shrink-0 overflow-hidden rounded-full ring-2 ring-card"><Image src={valueImages[index % valueImages.length]} alt="" fill className="object-cover transition-transform duration-500 group-hover:scale-110" /></div><div className="flex min-w-0 items-center gap-2"><Icon className="size-4 shrink-0 text-accent" aria-hidden="true" /><h3 className="whitespace-nowrap font-serif text-3xl font-semibold leading-tight text-accent">{value}</h3></div></article>
            })}
          </div>
        </div>
      </div>
