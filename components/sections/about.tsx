    ...
    <section id="about" className="bg-background py-20 lg:py-28">
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 className={`mt-0 text-balance text-lg font-extrabold leading-tight tracking-tight text-guiding-pink ${locale === "ko" ? "font-korean-serif text-4xl sm:text-5xl lg:text-6xl" : "font-serif text-4xl sm:text-5xl lg:text-6xl"}`}>{title}</h2>
        ...
        <div className="mt-16 grid items-stretch gap-4 lg:grid-cols-[1.45fr_.55fr]">
          ...
          <div className="grid gap-3">
            {values.length > 0 ? values.map((value, index) => {
              ...
              return <article key={value.id} className="group flex min-h-32 items-center gap-3 rounded-2xl bg-accent/10 px-3 py-3 transition-all duration-500 hover:-translate-y-1 hover:bg-accent/15"><div className="relative size-36 shrink-0 overflow-hidden rounded-full ring-2 ring-card"><Image src={valueImages[index % valueImages.length]} alt="" fill className="object-cover transition-transform duration-500 group-hover:scale-110" /></div><div className="flex min-w-0 items-center gap-2"><Icon className="size-5 shrink-0 text-accent" aria-hidden="true" /><h3 className="font-serif text-2xl font-semibold leading-tight text-accent">{text}</h3></div></article>
            }) : t.about.values.map((value, index) => {
              const Icon = fallbackIcons[index]
              return <article key={value} className="group flex min-h-32 items-center gap-3 rounded-2xl bg-accent/10 px-3 py-3 transition-all duration-500 hover:-translate-y-1 hover:bg-accent/15"><div className="relative size-36 shrink-0 overflow-hidden rounded-full ring-2 ring-card"><Image src={valueImages[index % valueImages.length]} alt="" fill className="object-cover transition-transform duration-500 group-hover:scale-110" /></div><div className="flex min-w-0 items-center gap-2"><Icon className="size-4 shrink-0 text-accent" aria-hidden="true" /><h3 className="font-serif text-2xl font-semibold leading-tight text-accent">{value}</h3></div></article>
            })}
          </div>
        </div>
      </div>
