      ...
      <div className="mx-auto grid max-w-[96rem] px-3 sm:px-5 lg:px-10 lg:grid-cols-[.85fr_1.15fr]">
        <div className="flex flex-col justify-center px-5 py-8 sm:px-8 sm:py-10 lg:px-10 lg:py-8">
          {content.visible && <>
            ...
            <div className="reveal-up reveal-delay-3 mt-9 flex flex-wrap items-center gap-4"><Button onClick={() => openBooking()} className={`h-12 ${locale === "ko" ? "w-24 text-2xl font-korean-sans" : locale === "en" ? "w-24 text-lg" : "w-32 text-xl"} shrink-0 rounded-full bg-accent px-4 font-extrabold text-accent-foreground hover:bg-accent/90`}>{locale === "en" ? "BOOK NOW" : localized(content.ctaEn, content.ctaKo, content.ctaVi)}</Button><a href="#about" className="flex items-center gap-2 text-sm font-medium text-foreground"><ArrowDown className="size-4" />{t.hero.hours}</a></div>
          </>}
        </div>
