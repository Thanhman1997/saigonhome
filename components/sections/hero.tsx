      ...
      <div className="mx-auto grid max-w-[96rem] px-3 sm:px-5 lg:px-10 lg:grid-cols-[.85fr_1.15fr]">
        <div className="flex flex-col justify-center px-5 py-8 sm:px-8 sm:py-10 lg:px-10 lg:py-8">
          {content.visible && <>
            ...
            <h1 className={`reveal-up reveal-delay-1 max-w-3xl text-balance font-sans font-light tracking-tight ${locale === "ko" ? "font-korean-sans" : ""}`} style={{ fontSize: locale === "ko" ? "clamp(1.9rem, 4.1vw, 3.9rem)" : locale === "en" ? "clamp(2rem, 4.3vw, 4.1rem)" : "clamp(2.2rem, 4.8vw, 4.5rem)", lineHeight: 1.06 }}>{localized(content.titleLine1En, content.titleLine1Ko, content.titleLine1Vi)}<br /><span className="not-italic text-accent">{localized(content.titleLine2En, content.titleLine2Ko, content.titleLine2Vi)}</span></h1>
