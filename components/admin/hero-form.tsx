...
export function HeroForm({ hero }: { hero: Hero }) {
  ...
  const autosaveStatus = useAdminAutosave("hero", "all", draftValues)
  ...
  return <form action={action} onChange={(event) => { const form = event.currentTarget; setDraftValues(Object.fromEntries(new FormData(form).entries()) as Record<string, string>) }} className="flex flex-col gap-6">
    ...
    <div className="flex flex-wrap items-center gap-3"><span className="text-sm text-muted-foreground" aria-live="polite">{autosaveStatus === "saving" ? "Saving..." : autosaveStatus === "saved" ? "Saved ✓" : autosaveStatus === "error" ? "Draft save failed" : ""}</span>
