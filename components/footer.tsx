...
export function Footer({ contactInfo }: { contactInfo: ContactInfo }) {
  ...
  return (
    <footer className="border-t border-border bg-secondary px-5 py-10 font-sans text-sm text-foreground lg:px-8">
      <div className="mx-auto grid max-w-6xl gap-2 md:grid-cols-3 md:translate-x-[1.5cm]">
        <div>
          <h2 className="font-sans text-xl font-extrabold uppercase tracking-[0.12em] text-guiding-pink">{footerCopy.services}</h2>
          <nav className="mt-4 flex flex-col gap-2 text-xl font-semibold leading-8 text-muted-foreground" aria-label="Services navigation">
            {footerCopy.serviceLinks.map((label) => <a key={label} href="#services" className="transition-colors hover:text-accent">{label}</a>)}
          </nav>
        </div>
        <div>
          <h2 className="font-sans text-xl font-extrabold uppercase tracking-[0.12em] text-guiding-pink">{footerCopy.about}</h2>
          <nav className="mt-4 flex flex-col gap-2 text-xl font-semibold leading-8 text-muted-foreground" aria-label="About navigation">
        ...
        <div id="contact" className="text-foreground">
          <h2 className="font-sans text-xl font-extrabold uppercase tracking-[0.12em] text-guiding-pink">{footerCopy.working}</h2>
          <div className="mt-2 flex items-start gap-3">
            <Clock3 className="mt-1 size-6 shrink-0 text-accent" strokeWidth={1.6} aria-hidden="true" />
            <div className="flex flex-col gap-1 text-xl font-semibold leading-8 text-muted-foreground">
              <div>
          ...
          <div className="mt-2 flex items-center gap-3">
            ...
            <p className="text-xl font-semibold leading-8 text-muted-foreground">{footerCopy.note.split("\n").map((line) => <span key={line}>{line}<br /></span>)}</p>
          </div>
        </div>
      </div>
